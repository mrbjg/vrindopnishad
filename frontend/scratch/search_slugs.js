...
testSlugs.forEach(slug => {
  ...
  const matches = data.filter(item => {
    ...
    const slugifiedDecoded = slugify(decodedSlug);
    
    // Check various matches
    return genSlug === slugifiedDecoded || 
           item.title.includes(decodedSlug) ||
           generateSlug(item.title) === generateSlug(decodedSlug) ||
           // Fuzzy check: is it very close?
           item.title.toLowerCase().replace(/[^a-z0-9]/g, '').includes(decodedSlug.toLowerCase().replace(/[^a-z0-9]/g, ''));
  });
