...
async def explain_content(req: ExplainRequest):
    ...
    system_prompt = (
        "You are a humble Brajwasi devotee of Shri Radha-Krishna. "
        "Your demeanor is filled with deep devotion (Bhakti), humility, sweetness, and reverence. "
        "Use terms like 'जय श्री राधे' (Jai Shri Radhe), 'श्यामा-श्याम' (Shyama-Shyam), 'ठाकुर जी' (Thakur Ji), 'वृ्राष्ट्रीय रस' (Vrindavan Ras) where appropriate. "
        "Your task is to explain the given verse (Sanskrit or Braj Bhasha) to a seeker. "
        "You will be given a reference explanation. Do NOT copy the reference explanation verbatim, and do NOT repeat its sentences. "
        "Instead, use the reference explanation as a spiritual guide (RAG reference), digest the meaning, and rewrite a fresh, copyright-safe, highly devotional explanation. "
        "Reveal the deeper nectar and inner emotional mood (bhava) of the verse. "
        "Always respond in the requested language: {language_name}.\n"
        "Make sure to structure your explanation beautifully with markdown:\n"
        "- Start with a loving devotional greeting.\n"
        "- Provide the word-by-word or simplified translation (सरल भावार्थ).\n"
        "- Elaborate on the deeper devotional meaning (दिव्य व्याख्या / रस भाव) of the verse."
    ).replace("{language_name}", lang_name)
    
    user_prompt = (
        f"Verse Title: {req.title or ''}\n"
        f"Author (Saint): {req.author or ''}\n"
        f"Original Verse:\n{req.sanskrit_text}\n\n"
        f"Reference Context (Do not copy verbatim, rewrite completely):\n{req.hindi_text or ''}\n"
    )

    groq_api_key = os.environ.get('GROQ_API_KEY')
    gemini_api_key = os.environ.get('GEMINI_API_KEY') or 'AQ.Ab8RN6ISfxlOzzqkxqjaXsseTIP6XVHKK2yTjCCfmVFpxvMyvQ'

    # Try Groq first
    if groq_api_key:
        logger.info("Attempting to explain using Groq API...")
    ...
    # Fallback to Gemini
    if gemini_api_key:
        logger.info("Attempting to explain using Gemini API...")
        ...
        try:
            ...
            if resp.status_code == 200:
                res = resp.json()
                explanation = res['candidates'][0]['content']['parts'][0]['text']
                return {"success": True, "explanation": explanation}
            else:
    ...
    default_msg = (
        "जय श्री राधे! क्षमा करें, ठाकुर जी की कृपा से व्याख्या अभी उपलब्ध नहीं हो पा रही है। "
        "कृपया कुछ समय पश्चात पुनः प्रयास करें।\n\n"
        f"मूल श्लोक:\n{req.sanskrit_text}"
    )
    return {"success": False, "explanation": default_msg, "message": "AI services are temporarily offline."}
